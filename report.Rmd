---
title: "Relationship between the Federal Funds Rate and Inflation"
author: "Sean Cannon"
...
```{r}
# Transform variables to be I(0)
ndata <- fred_dataw %>% dplyr::select(c(FEDFUNDS,PCEPI)) %>%
  mutate(infl = 1200*log(PCEPI/lag(PCEPI))) %>%              # in percent/month
  mutate(dinfl = infl - lag(infl,1)) %>%                     # differenced inflation
  mutate(fedfunds = FEDFUNDS - lag(FEDFUNDS)) %>%            # differenced fedfunds
  dplyr::select(dinfl, fedfunds) %>% 
  tidyr::drop_na()

zdata <- as.ts(ndata, start = c(1960, 1), frequency = 12)
...
ndatam <- melt(ndata, "Month")
ggplot(ndatam, aes(Month, value)) +
  geom_line() +
  facet_wrap(~variable, scales = "free", ncol = 2)

# Compare ACF plots before and after transformation
fred_dataw %>% ACF(FEDFUNDS) %>%
  autoplot()
ndata %>% ACF(fedfunds) %>%
  autoplot()
```
...
```{r}
cat("H0: fedfunds does not cause dinfl,  H1: it does \n")
grangertest(fedfunds ~ dinfl,order=p,data=zdata)
```

```{r}
cat("H0: dinfl does not cause fedfunds,  H1: it does \n")
grangertest(dinfl ~ fedfunds,order=p,data=zdata)
```
